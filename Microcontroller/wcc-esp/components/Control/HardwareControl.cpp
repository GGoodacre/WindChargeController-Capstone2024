    ...
    Control(LEDC_TIMER_8_BIT, PWM_FREQUENCY, LEDC_TIMER_0, MAX_CURRENT, R_SHUNT)
{
    ...
    _pwm_values[PWM_RECTIFIER].pid = QuickPID(&_devices[DEVICEINDEX_R4_RECTIFIED].vbus, &_pwm_values[PWM_RECTIFIER].duty_cycle, &_pwm_values[PWM_RECTIFIER].setpoint);
    ...
    _pwm_values[PWM_SEPIC].pid = QuickPID(&_sepic_pwm_cv, &_pwm_values[PWM_SEPIC].duty_cycle, &_pwm_values[PWM_SEPIC].setpoint);
    _pwm_values[PWM_SEPIC].pid.SetTunings(SEPIC_Kp, SEPIC_Ki, SEPIC_Kd);
    _pwm_values[PWM_SEPIC].pid.SetMode(1);
    _pwm_values[PWM_SEPIC].pid.SetOutputLimits(10, 90);
    ...
    _devices[DEVICEINDEX_R8_BATTERY].en = true;
...
{ 
    _sepic_pwm_cv = (SEPIC_SETPOINT + diode_drop) / (SEPIC_SETPOINT + diode_drop + _devices[DEVICEINDEX_R4_RECTIFIED].vbus);
