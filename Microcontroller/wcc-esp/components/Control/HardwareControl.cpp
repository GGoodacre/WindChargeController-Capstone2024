    ...
    Control(LEDC_TIMER_8_BIT, PWM_FREQUENCY, LEDC_TIMER_0, MAX_CURRENT, R_SHUNT)
{
    ...
    _pwm_values[PWM_RECTIFIER].channel = LEDC_CHANNEL_0;
    ...
    _pwm_values[PWM_SEPIC].channel = LEDC_CHANNEL_1;
    _pwm_values[PWM_SEPIC].device_pin = SEPIC_PIN;
    _pwm_values[PWM_SEPIC].invert = 1;
    ...
    _pwm_values[PWM_SEPIC].pid.SetMode(0);

    _pwm_values[PWM_PS1].duty_cycle = 0;
    _pwm_values[PWM_PS1].channel = LEDC_CHANNEL_3;
    _pwm_values[PWM_PS1].device_pin = PS1_PIN;
    _pwm_values[PWM_PS1].invert = 1;
    ...
    _pwm_values[PWM_PS2].duty_cycle = 100;
    _pwm_values[PWM_PS2].channel = LEDC_CHANNEL_4;
...
{ 
    _pwm_values[PWM_SEPIC].pid.Compute();
    ...
    {
        if(_pwm_values[i].duty_cycle > 100) _pwm_values[i].duty_cycle = 100;
        if(_pwm_values[i].duty_cycle < 0) _pwm_values[i].duty_cycle = 0;
